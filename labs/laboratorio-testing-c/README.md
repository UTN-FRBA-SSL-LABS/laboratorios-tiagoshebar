...
> R: Inicializa el carrito antes de usarlo y deja sus datos en un estado valido y vacio. Hay que llamarla antes de usar el carrito porque si no se inicializa puede tener valores de memoria que provoquen errores cuando se cuentan los productos.
...
> R: 24, total += c->items[i].precio;  /* BUG: falta multiplicar por cantidad */

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería sumar el precio de cada producto multiplicado por su cantidad.
...
> R: Falla, el test esperaba que el quinto carrito_agregar devolviera 0, pero devolvio 1 y se produce un Segmentation fault porque el carrito intenta agregar un producto cuando ya está lleno.
...
> R: Si hay líneas con #####: las líneas 29 y 30, que corresponden a carrito_descuento() no se ejecutaron porque los tests unitarios usados para generar la cobertura no llaman a esa función.
...
> R: Un test unitario verifica una función de forma aislada, mientras que un test de integración verifica que todas las funciones trabajen correctamente en conjunto. El test unitario con test_total_con_cantidad detectado primero el bug de carrito_total. 
...
> R: En C los arrays no controlan automáticamente que se acceda dentro de sus límites. Por eso escribir fuera del array puede sobrescribir otras partes de la memoria y provocar errores o un segmentation fault. La diferencia con Python o Java es que los accesos fuera de los límites de una lista o array son controlados y generan una excepción en lugar de escribir directamente en memoria.
...
> R: Es mejor porque los tests permiten comprobar si el programa funciona correctamente, permiten encontrar errores automáticamente y verificar después que una corrección no haya generado nuevos problemas.
...
> R: El primer test no detectó el bug porque cuando la cantidad es 1, multiplicar el precio por la cantidad da el mismo resultado que usar solamente el precio. Paso que 350 × 1 = 350. En cambio, con cantidad 2, debería dar 350 × 2 = 700, pero la función devolvía 350 y entonces el segundo test detectó el error.
